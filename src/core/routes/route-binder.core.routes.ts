...
import type { Method } from "../types/generic-routes.types.js";

export class RoutesBinder<T extends Record<string, RequestHandler>> {
    ...
    arg.forEach(({ method, path, middlewares, controller }) => {
      ...
      if (typeof handler !== "function") {
        throw CustomError.internalError(
          `El controlador '${String(controller)}' no es una función.`
        );
      }
